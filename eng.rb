...
########################
def gg
	gg_page = TkRoot.new do
		title "Language-path(eng)"
		minsize(400,400)
	end	
	gg_page_again = TkButton.new(gg_page) do
		text "Again"
		pack("side"=>"right", "padx"=> "50", "pady"=> "10")
		pack("side"=>"right", "padx"=> "50", "pady"=> "10")
		command (proc {start})
	end
	gg_page_exit = TkButton.new(gg_page) do
		text "exit"
		pack("side"=>"right", "padx"=> "50", "pady"=> "10")
		command(proc {exit})
	end
...
def start
	...
	########################
	########################
	eng_page = TkRoot.new do
		title "Language-path(eng)"
		minsize(400,400)
	end

	eng_page_lbl = TkLabel.new(eng_page) do  
		text "English"
	 	pack("padx"=> "0", "pady"=> "10")
	end

	eng_page_key = TkButton.new(eng_page) do
		text key
		pack('padx'=>0	,'pady'=>30)
	end

	value1 = TkButton.new(eng_page) do
		text v1
		if v1 == true_value
			command (proc {start})
		end
	...
	value2 = TkButton.new(eng_page) do
		text v2
		if v2 == true_value
			command (proc {start})
		end
	...
	value3 = TkButton.new(eng_page) do
		text v3
		if v3 == true_value
			command (proc {start})
		end
